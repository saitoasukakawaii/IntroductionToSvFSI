这一段是对svFSI给出的svFSI_master.inp文件进行简单翻译，方便快速使用，原文件如下链接

> https://github.com/SimVascular/svFSI/blob/JOSS_paper/svFSI_master.inp

```yaml
#---------------------------------------------------------------------
#  本文给出了svFSI的输入配置文件的所有可能选项，使用者根据使用方式自行配置
#
#  通常，输入文件包括三个部分:
#     1.  模拟参数
#     2.  网格信息
#     3.  求解的方程信息
#
#  模拟参数部分包括：空间维度、时间步控制、结果输出、断点重算等；网格信息包括几何部分、纤维生长方向、初始值、预紧力；方程信息包括求解的方程、是否耦合、边界条件、线性或非线性求解器、输出控制
#
#  参数设置通过“参数名: 值”的方式设置，如下
#
